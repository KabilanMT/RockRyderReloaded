...
public class Bread_Upgrade : MonoBehaviour
{
    private float pitch;
    ...
    void Start()
    {
        curpos = bread.transform.position;
        hov = bread.transform.position + Vector3.up * amountToMove;
        rest = curpos;
    ...
    void Update()
    {
        ...
        bread.transform.position = Vector3.MoveTowards(bread.transform.position, curpos, speed * Time.deltaTime);
    }
    private void OnMouseEnter()
    {
        textHandler.set_bread();
        curpos = hov;
    ...
    private void OnMouseDown()
    {
        ...
        if (payment.getMoney() < payment.getBreadCost() || payment.getBreadEaten() >= payment.getBreadMax())
        {
            pitch = Random.Range(0.0f, 1.0f);
            Debug.Log(pitch);
            audioSource.pitch = pitch + offset;
            audioSource.PlayOneShot(hover, 1.0F);
            if (payment.getMoney() < payment.getBreadCost())
            {
        ...
        else
        {
            ...
            payment.addBreadEaten();
            payment.subMoney(payment.getBreadCost());
            payment.updateBreadCost();
        }
    }
}
